...
한글 입력 중 **T(ㅅ)·O(ㅐ) 키를 뗀 직후 불필요한 ‘다음 곡’ 명령이 끼어들어 자모가 분리되는 특정 현상**을 막는 Windows 보정 도구입니다.
...
Windows 11 x64, .NET Framework 4.x 환경용입니다. 한 노트북에서 실제 타이핑으로 효과를 확인한 제한적인 우회 조치이며, 모든 한글 입력 오류를 고치는 범용 도구가 아닙니다. 최초의 불필요한 신호를 생성한 프로그램·드라이버·장치는 아직 특정하지 못했습니다.
...
- T, O 또는 한/영 키를 뗀 뒤 150ms 이내
- Windows 가상 키 `VK_MEDIA_NEXT_TRACK` (`0xB0`)
- 스캔 코드 `0`
- Windows의 `LLKHF_INJECTED` 표시가 있는 입력

원래 T/O/한영 키와 나머지 키는 전달합니다. 차단한 키 쌍의 반복·해제도 함께 처리합니다. 정상 프로그램이 같은 방식의 미디어 명령을 이 짧은 시간 안에 보내면 함께 차단될 수 있습니다.

## 검증과 한계

- 실제 예문 5회에서 자모 분리 0개를 확인했습니다.
- 해당 검사에서 불필요한 미디어 신호 19회 차단을 확인했습니다.
- 순수 정책 시험 25개를 통과했습니다.
...
A narrow Windows workaround for unwanted **injected Media Next Track events immediately after T, O or Hangul key release**, which interrupted Korean IME composition on one tested laptop. It does not identify or repair the underlying source and is not a universal Korean keyboard fix.
